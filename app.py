...
# All 100+ Commands Database
COMMANDS_DATA = {
    "group_management": [
        "/groupname newname - Change group name",
        "/grouplock - Lock group name",
        "/groupunlock - Unlock group name",
        "/removemember @user - Remove member",
        "/memberinfo @user - Get member info",
    ...
    "auto_reply": [
        ...
        "/setreply message - Set custom reply",
        "/getreply - Check current reply",
        "/replytoall on/off - Reply to all or mentions",
        "/delayreplied seconds - Set reply delay",
        "/replyemoji emoji - Add emoji to reply",
        "/replydisable @user - Don't reply to user",
        "/replyenable @user - Enable reply for user",
        "/clearreplyhist - Clear reply history"
    ],
    "youtube": [
        "/yt search - Search YouTube video",
        "/ytplay video_name - Play video",
        "/ytrandom - Random video",
        "/yttrending - Get trending",
        "/ytstop - Stop video",
        "/ytdownload url - Download video",
        "/ytinfo url - Get video info",
        "/ytsubtitle url - Download subtitles",
        "/ytplaylist url - Get playlist info",
        "/ytsearch latest - Latest videos"
    ],
    "messaging": [
        "/broadcast message - Send to all",
        "/dm @user message - Send DM",
        "/reply @user message - Reply to user",
        "/mention @user - Mention user",
        "/announce message - Announce",
        "/silent message - Silent message",
        "/spam @user message - Spam user",
        "/stopspam - Stop spam",
        "/quote @user - Quote message",
    ...
    "admin_commands": [
        ...
        "/muteuser @user seconds - Mute user",
        "/unmuteuser @user - Unmute user",
    ...
    "text_tools": [
        ...
        "/bold text - Make **bold**",
        "/italic text - Make *italic*",
        "/strike text - Strikethrough",
        "/morse text - Morse code",
        "/binary text - Binary code",
        "/hex text - Hex conversion",
        "/ascii text - ASCII values"
    ],
    "time_date": [
        "/time - Current time",
        "/date - Current date",
        ...
        "/daysleft event - Days left",
        "/clock timezone - World clock",
        "/timer seconds - Set timer",
        "/reminder message - Set reminder",
    ...
    "user_info": [
        ...
        "/bio @user - User bio",
        "/userjoined @user - When joined",
        "/lastactive @user - Last activity",
        "/isverified @user - Is verified",
        "/getavatar @user - Profile picture"
    ]
}
...
⏱️ Uptime: {uptime()}"""
        ...
        for cat, cmds in COMMANDS_DATA.items():
            help_text += f"• {cat.replace('_', ' ').title()}: {len(cmds)} commands\n"
        return help_text
    ...
    # Spam (Admin)
        ...
        STATS["commands_executed"] += 1
        return f"⚠️ Spam command activated!"
...
def run_bot(session_token, wm, gids, dly, pol, ucn, ecmd, admin_ids):
    ...
    log("🎉 Bot running with 100+ COMMANDS! 🚀")
                            ...
                            if is_admin:
                                if text.lower().startswith('/spam '):
                                    ...
                                    if len(parts) == 3:
                                        ...
                                        log(f"⚡ Spam activated")
                                
                                elif text.lower() == '/stopspam':
                                    BOT_CONFIG["spam_active"][gid] = False
                                    log("✅ Spam stopped")
...
<head>
    ...
    <style>
        * {margin:0;padding:0;box-sizing:border-box;}
        ...
        .header {text-align:center;margin-bottom:40px;padding:30px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:15px;box-shadow:0 8px 25px rgba(102,126,234,0.4);}
        .header h1 {font-size:2.5rem;margin-bottom:10px;text-shadow:2px 2px 4px rgba(0,0,0,0.3);}
        .header p {font-size:1.1rem;opacity:0.9;}
        .status-bar {display:flex;justify-content:space-between;align-items:center;padding:20px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:12px;margin-bottom:30px;box-shadow:0 8px 25px rgba(102,126,234,0.3);}
        .status-bar.status-running {background:linear-gradient(135deg,#10b981 0%,#059669 100%);}
        .status-bar.status-stopped {background:linear-gradient(135deg,#ef4444 0%,#dc2626 100%);}
        .status-dot {width:15px;height:15px;border-radius:50%;background:#ef4444;animation:pulse 2s infinite;margin-right:10px;}
        .status-running .status-dot {background:#10b981;}
        @keyframes pulse {0%,100%{opacity:1;} 50%{opacity:0.5;}}
        .content {background:rgba(255,255,255,0.05);border-radius:15px;padding:30px;backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.1);}
        .form-grid {display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:20px;margin-bottom:30px;}
        .form-group {display:flex;flex-direction:column;}
        .form-group.full {grid-column:1/-1;}
        .form-group label {font-weight:600;margin-bottom:8px;color:#a0d8ff;font-size:0.95rem;}
        .form-group input,.form-group textarea {padding:12px;background:rgba(255,255,255,0.1);border:2px solid rgba(102,126,234,0.3);border-radius:8px;color:#fff;font-size:1rem;transition:all 0.3s;}
        .form-group input:focus,.form-group textarea:focus {outline:none;border-color:#667eea;background:rgba(102,126,234,0.2);}
        .checkbox-group {display:flex;align-items:center;padding:15px;background:rgba(102,126,234,0.1);border-radius:8px;cursor:pointer;border:2px solid rgba(102,126,234,0.3);}
        .checkbox-group input {width:20px;height:20px;cursor:pointer;accent-color:#667eea;}
        .admin-section {background:linear-gradient(135deg,rgba(180,51,9,0.2) 0%,rgba(217,119,6,0.2) 100%);padding:20px;border-radius:12px;border-left:5px solid #f59e0b;margin-bottom:30px;}
        .admin-section h3 {color:#fbbf24;margin-bottom:15px;}
        .controls {display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:15px;margin-bottom:30px;}
        .btn {padding:12px 24px;border:none;border-radius:8px;font-weight:600;cursor:pointer;transition:all 0.3s;font-size:1rem;display:flex;align-items:center;justify-content:center;gap:8px;}
        ...
        .btn-stop {background:linear-gradient(135deg,#ef4444 0%,#dc2626 100%);color:#fff;}
        .btn-stop:hover {transform:translateY(-2px);box-shadow:0 8px 20px rgba(239,68,68,0.4);}
        .btn-clear {background:linear-gradient(135deg,#6b7280 0%,#4b5563 100%);color:#fff;}
        .btn-clear:hover {transform:translateY(-2px);}
        .logs-container {background:rgba(0,0,0,0.4);border-radius:12px;padding:20px;border:1px solid rgba(102,126,234,0.3);margin-top:30px;max-height:400px;overflow-y:auto;}
        #logs {font-family:'Courier New',monospace;font-size:0.9rem;line-height:1.6;white-space:pre-wrap;color:#a0d8ff;}
        .stats-grid {display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:20px;margin-bottom:30px;}
        .stat-card {background:linear-gradient(135deg,rgba(102,126,234,0.2) 0%,rgba(118,75,162,0.2) 100%);padding:20px;border-radius:12px;border:1px solid rgba(102,126,234,0.3);text-align:center;}
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>✨ 100+ Commands • YouTube • Auto-Reply • Group Management</p>
        </div>
            ...
            <form id="botForm">
                <div class="form-grid">
                    <div class="form-group">
                        <label><i class="fas fa-key"></i> Session Token *</label>
...
Follow rules! 👮</textarea>
                ...
                <div class="admin-section">
                    <h3 style="color:#fbbf24;"><i class="fas fa-crown"></i> 👑 Admin Commands</h3>
                    <div style="font-size:0.95rem;color:#fcd34d;line-height:1.8;">
                        <strong>🔧 Group:</strong> /groupname, /grouplock, /groupunlock<br>
            ...
            <div class="logs-container">
                <div style="display:flex;justify-content:space-between;align-items:center;color:#a0d8ff;margin-bottom:20px;font-weight:600;border-bottom:2px solid rgba(102,126,234,0.3);padding-bottom:10px;">
                    ...
                    <button onclick="clearLogs()" style="background:#667eea;color:white;border:none;padding:8px 15px;border-radius:6px;cursor:pointer;font-weight:600;">Clear</button>
                </div>
